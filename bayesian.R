...
### POSTERIOR ###
# Use equation 1.10, 1.11 and 1.12 from project thesis
...
contPlot = ggplot(df, aes(x=x, y=y)) + 
  ...
  coord_fixed(xlim = c(mu[1]-4*sigma[1,1],mu[1]+4*sigma[1,1]),
                               ylim = c(mu[2]-4*sigma[2,2], mu[2]+4*sigma[2,2]), ratio = 1) + 
...
xx = seq(from=min(X[,2]), to = max(X[,2]), length.out = 100)
...
#grid.arrange(contPlot, regPlot, ncol=2)
contPlot
